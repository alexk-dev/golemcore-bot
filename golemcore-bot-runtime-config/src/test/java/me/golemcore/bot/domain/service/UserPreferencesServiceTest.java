...
class UserPreferencesServiceTest {
    ...
    private LocalizationPort localizationPort;
    private UserPreferencesService service;

    @BeforeEach
    void setUp() {
        userPreferencesStorePort = mock(UserPreferencesStorePort.class);
        localizationPort = mock(LocalizationPort.class);
        when(localizationPort.defaultLanguage()).thenReturn(LANG_EN);
        service = new UserPreferencesService(userPreferencesStorePort, localizationPort);
    }
    ...
    void getPreferencesCreatesDefaultWhenNoSaved() {
        when(userPreferencesStorePort.loadPreferences()).thenThrow(new IllegalStateException(NOT_FOUND));

        UserPreferences prefs = service.getPreferences();

        assertNotNull(prefs);
        assertEquals(LANG_EN, prefs.getLanguage());
        verify(localizationPort).setLanguage(LANG_EN);
    }
    ...
    void getPreferencesHandlesBlankJson() {
        when(userPreferencesStorePort.loadPreferences()).thenReturn(Optional.empty());
    ...
    void getPreferencesHandlesMalformedJson() {
        when(userPreferencesStorePort.loadPreferences()).thenThrow(new IllegalStateException("malformed"));
    ...
    void getPreferencesHandlesSaveFailureOnCreation() {
        when(userPreferencesStorePort.loadPreferences()).thenThrow(new IllegalStateException(NOT_FOUND));
        doThrow(new IllegalStateException("disk full")).when(userPreferencesStorePort)
                .savePreferences(any(UserPreferences.class));
    ...
    void savePreferencesPersistsToStorage() {
        ...
        verify(localizationPort).setLanguage(LANG_RU);
    }

    @Test
    void savePreferencesThrowsOnStorageFailure() {
        doThrow(new IllegalStateException("disk error")).when(userPreferencesStorePort)
                .savePreferences(any(UserPreferences.class));

        UserPreferences prefs = UserPreferences.builder().language(LANG_EN).build();

        IllegalStateException ex = assertThrows(IllegalStateException.class, () -> service.savePreferences(prefs));
    ...
    void savePreferencesRollsBackOnStorageFailure() {
        ...
        doThrow(new IllegalStateException("disk full")).when(userPreferencesStorePort).savePreferences(newPrefs);
        ...
        verify(localizationPort, atLeastOnce()).setLanguage(LANG_EN);
    }
    ...
    void setLanguageUpdatesAndSaves() {
        ...
        verify(localizationPort, atLeastOnce()).setLanguage(LANG_RU);
    }
    ...
    void getMessageDelegatesToMessageService() {
        when(userPreferencesStorePort.loadPreferences()).thenThrow(new IllegalStateException(NOT_FOUND));
        when(localizationPort.getMessage("key", LANG_EN, "arg1")).thenReturn("Hello arg1");
