...
public class JsonHivePolicyStateAdapter implements ManagedPolicyStatePort {
    ...
    @Override
    public void save(ManagedPolicyBindingState state) {
        if (state == null) {
            throw new IllegalArgumentException("Hive policy binding state is required");
        }
        ManagedPolicyBindingState snapshot = copy(state);
        synchronized (lock) {
            try {
    ...
    private Optional<ManagedPolicyBindingState> loadState() {
        try {
            String json = storagePort.getText(PREFERENCES_DIR, POLICY_STATE_FILE).join();
            if (json == null || json.isBlank()) {
                return Optional.empty();
            }
            return Optional.of(objectMapper.readValue(json, ManagedPolicyBindingState.class));
    ...
    private ManagedPolicyBindingState copy(ManagedPolicyBindingState source) {
        if (source == null) {
            return null;
        }
