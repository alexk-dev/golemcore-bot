...
public class HiveInspectionPayloadMapper implements HiveInspectionPayloadPort {
    ...
    private Map<String, Object> toAttachmentPayload(SessionDetailView.AttachmentView attachment) {
        ...
        payload.put("url", resolveAttachmentUrl(attachment));
        payload.put("internalFilePath", attachment.getInternalFilePath());
    ...
    private String resolveAttachmentUrl(SessionDetailView.AttachmentView attachment) {
        if (attachment.getDirectUrl() != null) {
            return attachment.getDirectUrl();
        }
        if (attachment.getInternalFilePath() == null) {
            return null;
        }
        String encoded = URLEncoder.encode(attachment.getInternalFilePath(), StandardCharsets.UTF_8)
                .replace("+", "%20");
