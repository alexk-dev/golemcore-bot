...
public class SessionRetentionCleanupService {
    ...
    private boolean shouldRetainSession(AgentSession session, Set<String> protectedActiveSessionKeys,
        ...
        if (protectSessionsWithPlans && planService.hasActivePlans(sessionIdentity)) {
            return true;
        }
        if (protectSessionsWithDelayedActions
                && delayedSessionActionService.hasPendingActions(sessionIdentity.channelType(),
