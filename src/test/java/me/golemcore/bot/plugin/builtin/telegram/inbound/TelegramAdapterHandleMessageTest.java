...
class TelegramAdapterHandleMessageTest {
    ...
    void shouldRouteKnownCommandToRouter() throws Exception {
        when(commandRouter.hasCommand(COMMAND_HELP, "telegram")).thenReturn(true);
        ...
        verify(commandRouter).hasCommand(COMMAND_HELP, "telegram");
    ...
    void shouldRouteCommandWithArgs() throws Exception {
        when(commandRouter.hasCommand("compact", "telegram")).thenReturn(true);
    ...
    void shouldDelegateSettingsToMenuHandler() {
        ...
        verify(commandRouter, never()).hasCommand("settings", "telegram");
        verify(messageHandler, never()).accept(any());
    }
    ...
    void shouldDelegateMenuCommandToMenuHandler() {
        ...
        verify(menuHandler).sendMainMenu("100");
        verify(commandRouter, never()).hasCommand("menu", "telegram");
        verify(messageHandler, never()).accept(any());
    }
    ...
    void shouldDelegateSessionsCommandToMenuHandler() {
        ...
        verify(commandRouter, never()).hasCommand("sessions", "telegram");
        verify(messageHandler, never()).accept(any());
    }
    ...
    void shouldCreateNewTelegramSessionOnNewCommand() throws Exception {
        ...
        verify(commandRouter, never()).hasCommand("new", "telegram");
        verify(messageHandler, never()).accept(any());
    }
    ...
    void shouldHandleCommandFailure() throws Exception {
        when(commandRouter.hasCommand("failing", "telegram")).thenReturn(true);
    ...
    void shouldPassUnknownCommandAsRegularMessage() {
        when(commandRouter.hasCommand("unknown", "telegram")).thenReturn(false);
