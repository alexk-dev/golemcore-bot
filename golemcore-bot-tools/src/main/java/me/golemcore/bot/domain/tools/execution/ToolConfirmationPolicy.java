...
import me.golemcore.bot.domain.runtimeconfig.ToolRuntimeConfigView;
import org.springframework.stereotype.Component;

import java.util.Map;

/**
 * Policy engine determining which tool calls require explicit user confirmation before execution. Identifies
 * potentially destructive operations (file deletion, shell commands, skill deletion) and provides human-readable action
 * descriptions for confirmation prompts. Can be disabled via configuration while still detecting notable actions for
...
public class ToolConfirmationPolicy {
    ...
    private final ToolRuntimeConfigView runtimeConfigService;

    public ToolConfirmationPolicy(ToolRuntimeConfigView runtimeConfigService) {
    ...
    /**
     * Check if a tool call is a notable/dangerous action (regardless of enabled state). Used for informational
     * notifications when confirmation is disabled.
     */
