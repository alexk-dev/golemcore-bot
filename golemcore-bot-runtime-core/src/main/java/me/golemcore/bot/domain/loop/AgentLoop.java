...
/**
 * Core orchestrator managing the agent processing pipeline for incoming messages. Coordinates session management, rate
 * limiting, system pipeline execution, and channel responses. Systems are executed in order based on their @Order
 * annotation (InputSanitization -> SkillRouting -> ContextBuilding -> LlmExecution -> ToolExecution -> MemoryPersist ->
 * ResponseRouting). Manages typing indicators and handles async message processing.
 */
@Slf4j
public class AgentLoop {
    ...
    public AgentContext processMessage(Message message) {
        ...
        try (MdcSupport.Scope ignored = MdcSupport.withContext(mdcContext)) {
            ...
            AgentSession session = sessionService.getOrCreate(message.getChannelType(), message.getChatId());
            ...
            AgentContext context = AgentContext.builder().session(session)
                    .messages(buildContextMessages(session, message)).maxIterations(resolveMaxSkillTransitions())
                    .currentIteration(0).build();
    ...
    /**
     * Copies non-string metadata values into runtime attributes. Used for webhook schema maps that must be available to
     * multiple pipeline systems.
     */
        ...
        for (int iteration = 0; iteration < maxIterations; iteration++) {
            ...
            for (AgentSystem system : getSortedSystems()) {
                ...
                TraceContext systemSpan = startChildSpan(context, "system." + system.getName(), TraceSpanKind.INTERNAL,
                        Map.of("system.name", system.getName(), "system.order", system.getOrder(), "iteration",
                                iteration));
                try {
