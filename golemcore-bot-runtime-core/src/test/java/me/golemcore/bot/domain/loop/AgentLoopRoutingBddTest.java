...
class AgentLoopRoutingBddTest {
    ...
    void scenario_iterationLimit_shouldRouteSyntheticLimitMessageViaRoutingSystem() {
        ...
        // Use a terminal routing system so AgentLoop's marker check works.
        ResponseRoutingAgentSystem routing = responseRoutingSystem(List.of(channel), preferencesService);

        AgentLoop loop = createLoop(sessionPort, rateLimitPort, List.of(requester, routing), List.of(channel),
                mockRuntimeConfigService(2), preferencesService, llmPort, clock);

        Message inbound = Message.builder().role("user").content("hi").channelType(CHANNEL_TYPE).chatId("1")
    ...
    private record TestResponseRoutingSystem(ChannelRuntimePort channelRuntimePort)
        ...
        @Override
        public AgentContext process(AgentContext context) {
            me.golemcore.bot.domain.model.OutgoingResponse outgoing = context
                    .getAttribute(me.golemcore.bot.domain.model.ContextAttributes.OUTGOING_RESPONSE);
            ...
            ChannelDeliveryPort channel = channelRuntimePort.findChannel(context.getSession().getChannelType())
                    .orElse(null);
            if (channel != null) {
                channel.sendMessage(context.getSession().getChatId(), outgoing.getText(), Map.of());
            }
            return context;
        }
    }
