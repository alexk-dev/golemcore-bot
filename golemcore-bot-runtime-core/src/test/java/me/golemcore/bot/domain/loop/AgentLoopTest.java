...
class AgentLoopTest {
    ...
    void shouldResetTypedControlFlagsAndToolResultsBetweenIterations() {
        ...
        AgentLoop loop = createLoop(sessionPort, rateLimitPort, List.of(verifier), List.of(channel),
                mockRuntimeConfigService(2), preferencesService, llmPort, clock);
    ...
    void shouldUseTransportChatIdForTypingIndicator() {
        ...
        Clock clock = Clock.fixed(Instant.parse(FIXED_INSTANT), ZoneOffset.UTC);
        AgentSession session = AgentSession.builder().id("s1").channelType(CHANNEL_TYPE).chatId("conv-1")
                .messages(new ArrayList<>()).build();
        ...
        AgentLoop loop = createLoop(sessionPort, rateLimitPort, List.of(), List.of(channel),
                mockRuntimeConfigService(1), preferencesService, llmPort, clock);

        Message inbound = Message.builder().role(ROLE_USER).content("hi").channelType(CHANNEL_TYPE).chatId("conv-1")
                .senderId("u1").metadata(Map.of(ContextAttributes.TRANSPORT_CHAT_ID, "transport-99",
    ...
    void shouldPersistSessionIdentityMetadataFromInboundMessage() {
        ...
        Message inbound = Message.builder().role(ROLE_USER).content("identity bind").channelType(CHANNEL_TYPE)
                .chatId("conv-1").senderId("u1").metadata(Map.of(ContextAttributes.TRANSPORT_CHAT_ID, "transport-42",
                        ContextAttributes.CONVERSATION_KEY, "conv-1"))
                .timestamp(clock.instant()).build();
    ...
    void shouldStartRootTraceAndPropagateTraceContextIntoAgentContext() {
        ...
        Message inbound = Message.builder().role(ROLE_USER).content("trace me").channelType(CHANNEL_TYPE)
                .chatId("conv-1").senderId("u1")
                .metadata(Map.of(ContextAttributes.TRACE_ID, "trace-1", ContextAttributes.TRACE_SPAN_ID, "span-1",
                        ContextAttributes.TRACE_ROOT_KIND, TraceSpanKind.INGRESS.name(), ContextAttributes.TRACE_NAME,
                        "telegram.message"))
                .timestamp(clock.instant()).build();
    ...
    void shouldRecordSystemAndSessionSaveChildSpans() {
        ...
        AgentLoop loop = createLoop(sessionPort, rateLimitPort, List.of(tracedSystem), List.of(channel),
                mockRuntimeConfigService(1), preferencesService, llmPort, clock);

        Message inbound = Message.builder().role(ROLE_USER).content("trace systems").channelType(CHANNEL_TYPE)
                .chatId("conv-1").senderId("u1")
                .metadata(Map.of(ContextAttributes.TRACE_ID, "trace-1", ContextAttributes.TRACE_SPAN_ID, "span-1",
                        ContextAttributes.TRACE_ROOT_KIND, TraceSpanKind.INGRESS.name(), ContextAttributes.TRACE_NAME,
                        "telegram.message"))
        ...
        TraceSpanRecord systemSpan = trace.getSpans().stream().filter(span -> "system.traceable".equals(span.getName()))
                .findFirst().orElseThrow();
        assertEquals("span-1", systemSpan.getParentSpanId());
        assertEquals(TraceStatusCode.OK, systemSpan.getStatusCode());

        TraceSpanRecord saveSpan = trace.getSpans().stream().filter(span -> "session.save".equals(span.getName()))
                .findFirst().orElseThrow();
    ...
    void shouldCopyWebhookResponseSchemaMetadataIntoRuntimeAttributes() {
        ...
        Clock clock = Clock.fixed(Instant.parse(FIXED_INSTANT), ZoneOffset.UTC);
        AgentSession session = AgentSession.builder().id("webhook:hook:test").channelType("webhook").chatId("hook:test")
                .messages(new ArrayList<>()).build();
        ...
        AgentSystem inspector = new AgentSystem() {
            ...
            public AgentContext process(AgentContext context) {
                ...
                observedValidationTiers
                        .add(context.getAttribute(ContextAttributes.WEBHOOK_RESPONSE_VALIDATION_MODEL_TIER));
                return context;
            }
        };

        AgentLoop loop = createLoop(sessionPort, rateLimitPort, List.of(inspector), List.of(channel),
                mockRuntimeConfigService(1), preferencesService, llmPort, clock);

        Map<String, Object> schema = Map.of("type", "object");
        Message inbound = Message.builder().role(ROLE_USER).content("trace schema").channelType("webhook")
                .chatId("hook:test").senderId("u1")
                .metadata(Map.of(ContextAttributes.WEBHOOK_RESPONSE_JSON_SCHEMA, schema,
    ...
    void shouldKeepWebhookResponseSchemaInstructionsAcrossSkillTransitionIteration() {
        ...
        AgentSystem transitionSystem = new AgentSystem() {
            ...
            public AgentContext process(AgentContext context) {
                if (context.getCurrentIteration() == 0) {
                    context.getMessages().add(Message.builder().role("assistant")
                            .content("Intermediate answer without schema metadata").timestamp(clock.instant()).build());
        ...
        AgentLoop loop = createLoop(sessionPort, rateLimitPort, List.of(contextSystem, transitionSystem),
                List.of(channel), mockRuntimeConfigService(2), preferencesService, llmPort, clock);

        Message inbound = Message.builder().role(ROLE_USER).content("trace schema").channelType("webhook")
                .chatId("hook:test").senderId("u1")
                .metadata(Map.of(ContextAttributes.WEBHOOK_RESPONSE_JSON_SCHEMA_TEXT, "{\"type\":\"object\"}"))
                .timestamp(clock.instant()).build();
    ...
    void shouldRecordWebhookResponseSchemaInstructionTraceEvent() {
        ...
        AgentLoop loop = createLoop(sessionPort, rateLimitPort, List.of(contextSystem), List.of(channel),
                mockRuntimeConfigService(1), preferencesService, llmPort, clock);
        ...
        Message inbound = Message.builder().role(ROLE_USER).content("trace schema").channelType("webhook")
                ...
                .metadata(Map.of(ContextAttributes.TRACE_ID, "trace-1", ContextAttributes.TRACE_SPAN_ID, "span-1",
                        ContextAttributes.TRACE_ROOT_KIND, TraceSpanKind.INGRESS.name(), ContextAttributes.TRACE_NAME,
                        "webhook.agent", ContextAttributes.WEBHOOK_RESPONSE_JSON_SCHEMA_TEXT, "{\"type\":\"object\"}",
                        ContextAttributes.WEBHOOK_RESPONSE_VALIDATION_MODEL_TIER, "smart"))
                .timestamp(clock.instant()).build();
    ...
    void shouldSendUnsentLlmResponseAsFeedbackGuarantee() {
        ...
        AgentLoop loop = createLoop(sessionPort, rateLimitPort,
                List.of(system, createRoutingSystem(channel, preferencesService)), List.of(channel),
                mockRuntimeConfigService(1), preferencesService, llmPort, clock);
    ...
    void shouldSkipDisabledSystem() {
        ...
        AgentLoop loop = createLoop(sessionPort, rateLimitPort, List.of(disabledSystem), List.of(channel),
                mockRuntimeConfigService(1), preferencesService, llmPort, clock);
    ...
    void shouldRecordFailureEventOnSystemException() {
        ...
        AgentLoop loop = createLoop(sessionPort, rateLimitPort, List.of(throwingSystem, inspectorSystem),
                List.of(channel), mockRuntimeConfigService(1), preferencesService, llmPort, clock);
    ...
    void shouldCollectErrorsFromFailureEvents() {
        ...
        AgentSystem failureSystem = new AgentSystem() {
            ...
            @Override
            public AgentContext process(AgentContext context) {
                context.addFailure(new FailureEvent(FailureSource.LLM, "testLlm", FailureKind.EXCEPTION,
        ...
        AgentLoop loop = createLoop(sessionPort, rateLimitPort, List.of(failureSystem, routingSystem), List.of(channel),
                mockRuntimeConfigService(1), preferencesService, llmPort, clock);
    ...
    void shouldHandleNullMessagesInAutoModeCheck() {
        ...
        AgentLoop loop = createLoop(sessionPort, rateLimitPort, List.of(nullMessagesSystem, routingSystem),
                List.of(channel), mockRuntimeConfigService(1), preferencesService, llmPort, clock);
    ...
    void shouldTruncateNullInput() {
        ...
        Message inbound = Message.builder().role(ROLE_USER).content(null).channelType(CHANNEL_TYPE).chatId("1")
                .senderId("u1").timestamp(clock.instant()).build();
    ...
    void shouldEnsureFeedbackWhenRoutingOutcomeNotSent() {
        ...
        AgentSystem turnOutcomeSystem = new AgentSystem() {
            ...
            @Override
            public AgentContext process(AgentContext context) {
                RoutingOutcome routingOutcome = RoutingOutcome.builder().attempted(true).sentText(false).build();
                TurnOutcome outcome = TurnOutcome.builder().finishReason(FinishReason.ERROR)
                        .routingOutcome(routingOutcome).build();
    ...
    void shouldSkipFeedbackWhenTurnOutcomeReportsAttachmentDelivery() {
        ...
        AgentSystem turnOutcomeSystem = new AgentSystem() {
            ...
            @Override
            public AgentContext process(AgentContext context) {
                RoutingOutcome routingOutcome = RoutingOutcome.builder().attempted(true).sentText(false)
                        .sentAttachments(1).build();
                TurnOutcome outcome = TurnOutcome.builder().finishReason(FinishReason.SUCCESS)
                        .routingOutcome(routingOutcome).build();
                context.setTurnOutcome(outcome);
                return context;
            }
    ...
    void shouldSkipFeedbackWhenRoutingOutcomeAttributeReportsVoiceDelivery() {
        ...
        AgentSystem routingOutcomeAttributeSystem = new AgentSystem() {
            ...
            @Override
            public AgentContext process(AgentContext context) {
                RoutingOutcome routingOutcome = RoutingOutcome.builder().attempted(true).sentText(false).sentVoice(true)
                        .build();
    ...
    void shouldBypassRateLimitForAutoMode() {
        ...
        Message autoMsg = Message.builder().role(ROLE_USER).content("auto task").channelType(CHANNEL_TYPE).chatId("1")
                .senderId("auto").metadata(meta).timestamp(clock.instant()).build();
    ...
    void shouldBypassRateLimitAndRawHistoryForInternalRetryMessage() {
        ...
        Message internalMsg = Message.builder().role(ROLE_USER).content("Continue and finish").channelType(CHANNEL_TYPE)
                .chatId("1").senderId("internal").metadata(meta).timestamp(clock.instant()).build();
    ...
    void shouldCopyL5ResumeMetadataFromInternalDelayedRetryMessage() {
        ...
        Message internalMsg = Message.builder().role(ROLE_USER).content("Retry").channelType(CHANNEL_TYPE).chatId("1")
                .senderId("internal").metadata(meta).timestamp(clock.instant()).build();
    ...
    void shouldSkipFeedbackGuaranteeWhenInternalRetryAlreadyScheduled() {
        ...
        ResponseRoutingAgentSystem routing = createRoutingSystem(channel, preferencesService);

        AgentLoop loop = createLoop(sessionPort, rateLimitPort, List.of(retryScheduled, routing), List.of(channel),
                mockRuntimeConfigService(1), preferencesService, llmPort, clock);
    ...
    void shouldSkipFeedbackGuaranteeForAutoMode() {
        ...
        AgentLoop loop = createLoop(sessionPort, rateLimitPort, List.of(routing), List.of(channel),
                mockRuntimeConfigService(1), preferencesService, llmPort, clock);
    ...
    void shouldSendIterationLimitMessage() {
        ...
        AgentLoop loop = createLoop(sessionPort, rateLimitPort, List.of(alwaysTransition, routing), List.of(channel),
                mockRuntimeConfigService(1), preferencesService, llmPort, clock);
        ...
        AgentSystem attachmentsOnlySystem = new AgentSystem() {
            ...
            public AgentContext process(AgentContext context) {
                me.golemcore.bot.domain.model.Attachment attachment = me.golemcore.bot.domain.model.Attachment.builder()
                        .type(me.golemcore.bot.domain.model.Attachment.Type.IMAGE).data(new byte[] { 1, 2, 3 })
                        .filename("img.png").caption("img").build();
                context.setAttribute(ContextAttributes.OUTGOING_RESPONSE,
                        OutgoingResponse.builder().attachment(attachment).build());
                return context;
            }
        };
    ...
    void shouldFallbackToGenericFeedbackWhenLlmInterpretationTimesOut() {
        ...
        AgentLoop loop = createLoop(sessionPort, rateLimitPort, List.of(errorSystem, routing), List.of(channel),
                mockRuntimeConfigService(1), preferencesService, llmPort, clock);
    ...
    void shouldRecordReflectionCompletionOutcomeForAutoRunMessage() {
        ...
        AgentSession session = AgentSession.builder().id("s-auto").channelType(CHANNEL_TYPE).chatId("auto-chat")
                .messages(new ArrayList<>()).build();
        ...
        AgentSystem reflectionSystem = new AgentSystem() {
            ...
            public AgentContext process(AgentContext context) {
                ...
                context.setTurnOutcome(TurnOutcome.builder().assistantText("Reflected answer")
                        .finishReason(FinishReason.SUCCESS).build());
                return context;
            }
        };

        AgentLoop loop = createLoop(sessionPort, rateLimitPort, List.of(reflectionSystem), List.of(),
                mockRuntimeConfigService(1), preferencesService, llmPort, clock);

        Map<String, Object> metadata = new HashMap<>();
        metadata.put(ContextAttributes.AUTO_MODE, true);
        metadata.put(ContextAttributes.AUTO_RUN_ID, "run-1");
        Message inbound = Message.builder().role(ROLE_USER).content("reflect").channelType(CHANNEL_TYPE)
                .chatId("auto-chat").senderId("auto").metadata(metadata).timestamp(clock.instant()).build();
    ...
    void shouldRecordReflectionFailureOutcomeForAutoRunMessage() {
        ...
        AgentSession session = AgentSession.builder().id("s-auto-fail").channelType(CHANNEL_TYPE).chatId("auto-chat")
                .messages(new ArrayList<>()).build();
        when(sessionPort.getOrCreate(CHANNEL_TYPE, "auto-chat")).thenReturn(session);
        ...
        AgentSystem reflectionFailureSystem = new AgentSystem() {
            ...
            public AgentContext process(AgentContext context) {
                ...
                context.addFailure(new FailureEvent(FailureSource.LLM, "test-llm", FailureKind.EXCEPTION,
                        "Planner timeout", clock.instant()));
                context.setTurnOutcome(TurnOutcome.builder().finishReason(FinishReason.ERROR).build());
                return context;
            }
        };

        AgentLoop loop = createLoop(sessionPort, rateLimitPort, List.of(reflectionFailureSystem), List.of(),
                mockRuntimeConfigService(1), preferencesService, llmPort, clock);

        Map<String, Object> metadata = new HashMap<>();
        metadata.put(ContextAttributes.AUTO_MODE, true);
        metadata.put(ContextAttributes.AUTO_RUN_ID, "run-2");
        Message inbound = Message.builder().role(ROLE_USER).content("reflect fail").channelType(CHANNEL_TYPE)
                .chatId("auto-chat").senderId("auto").metadata(metadata).timestamp(clock.instant()).build();
    ...
    void shouldRecordExplicitSkillAndTierEventsOnSystemSpans() {
        ...
        AgentSystem applySystem = new AgentSystem() {
            ...
            public AgentContext process(AgentContext context) {
                context.setActiveSkill(
                        me.golemcore.bot.domain.model.Skill.builder().name("executor").modelTier("smart").build());
        ...
        AgentLoop loop = createLoop(sessionPort, rateLimitPort, List.of(requestSystem, applySystem, dynamicTierSystem),
                List.of(channel), mockRuntimeConfigService(1), preferencesService, llmPort, clock);

        Message inbound = Message.builder().role(ROLE_USER).content("trace transitions").channelType(CHANNEL_TYPE)
                .chatId("conv-1").senderId("u1")
                .metadata(Map.of(ContextAttributes.TRACE_ID, "trace-1", ContextAttributes.TRACE_SPAN_ID, "span-1",
                        ContextAttributes.TRACE_ROOT_KIND, TraceSpanKind.INGRESS.name(), ContextAttributes.TRACE_NAME,
                        "telegram.message"))
    ...
    void shouldRecordSessionStateSkillRestoreOnContextBuildingSpan() {
        ...
        Clock clock = Clock.fixed(Instant.parse(FIXED_INSTANT), ZoneOffset.UTC);
        AgentSession session = AgentSession.builder().id("s-restore").channelType(CHANNEL_TYPE).chatId("conv-restore")
                .messages(new ArrayList<>()).build();
        ...
        AgentSystem applySystem = new AgentSystem() {
            ...
            public AgentContext process(AgentContext context) {
                context.setActiveSkill(
                        me.golemcore.bot.domain.model.Skill.builder().name("reviewer").modelTier("smart").build());
        ...
        AgentLoop loop = createLoop(sessionPort, rateLimitPort, List.of(applySystem), List.of(channel),
                mockRuntimeConfigService(1), preferencesService, llmPort, clock);

        Message inbound = Message.builder().role(ROLE_USER).content("resume skill").channelType(CHANNEL_TYPE)
                .chatId("conv-restore").senderId("u1")
                .metadata(Map.of(ContextAttributes.TRACE_ID, "trace-restore", ContextAttributes.TRACE_SPAN_ID,
                        "span-restore", ContextAttributes.TRACE_ROOT_KIND, TraceSpanKind.INGRESS.name(),
    ...
    private static ResponseRoutingAgentSystem createRoutingSystem(ChannelPort channel,
        ...
        return responseRoutingSystem(List.of(channel), preferencesService);
    }

    private static ChannelRuntimePort runtime(List<ChannelPort> channels) {
        List<ChannelDeliveryPort> deliveryPorts = List.copyOf(channels);
        return new ChannelRuntimePort() {
            @Override
            public Optional<ChannelDeliveryPort> findChannel(String channelType) {
                return deliveryPorts.stream().filter(channel -> channel.getChannelType().equals(channelType))
                        .findFirst();
            }
    ...
    private static ResponseRoutingAgentSystem responseRoutingSystem(List<ChannelPort> channels,
            UserPreferencesService preferencesService) {
        return new TestResponseRoutingSystem(runtime(channels));
    }

    private record TestResponseRoutingSystem(ChannelRuntimePort channelRuntimePort)
            implements ResponseRoutingAgentSystem {

        @Override
        public String getName() {
            return "TestResponseRoutingSystem";
        }
        ...
        public AgentContext process(AgentContext context) {
            ...
            ChannelDeliveryPort channel = channelRuntimePort.findChannel(context.getSession().getChannelType())
                    .orElse(null);
            if (channel == null) {
                context.setAttribute(ContextAttributes.ROUTING_OUTCOME,
                        RoutingOutcome.builder().attempted(true).sentText(false).build());
                return context;
            }

            channel.sendMessage(chatId, outgoing.getText(), Map.of());
            context.setAttribute(ContextAttributes.ROUTING_OUTCOME,
                    RoutingOutcome.builder().attempted(true).sentText(true).build());
            return context;
        }
    }
    ...
    private static TraceSpanRecord findSpan(TraceRecord trace, String name) {
        return trace.getSpans().stream().filter(span -> name.equals(span.getName())).findFirst().orElseThrow();
    }

    private static TraceEventRecord findEvent(TraceSpanRecord span, String name) {
        return span.getEvents().stream().filter(event -> name.equals(event.getName())).findFirst().orElseThrow();
    }
}
