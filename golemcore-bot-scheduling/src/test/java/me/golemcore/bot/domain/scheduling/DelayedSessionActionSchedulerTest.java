...
class DelayedSessionActionSchedulerTest {
    ...
    void shouldRescheduleRetryableDispatchWithBackoff() {
        ...
        DelayedSessionAction action = DelayedSessionAction.builder().id("delay-1").kind(DelayedActionKind.RUN_LATER)
                .deliveryMode(DelayedActionDeliveryMode.INTERNAL_TURN).attempts(0).maxAttempts(4).build();
        when(delayedActionService.leaseDueActions(10)).thenReturn(List.of(action));
        when(delayedActionDispatcher.dispatch(action)).thenReturn(
                CompletableFuture.completedFuture(DelayedActionDispatcher.DispatchResult.retryable("temporary")));

        DelayedSessionActionScheduler scheduler = new DelayedSessionActionScheduler(delayedActionService,
                delayedActionDispatcher, runtimeConfigService, Clock.fixed(NOW, ZoneOffset.UTC));
        scheduler.init();
        try {
            scheduler.tick();

            verify(delayedActionService, timeout(2000)).rescheduleRetry(eq("delay-1"), eq(NOW.plusSeconds(30)),
                    eq("temporary"));
    ...
    void shouldMarkDeadLetterWhenRetryBudgetIsExhausted() {
        ...
        DelayedSessionAction action = DelayedSessionAction.builder().id("delay-2").kind(DelayedActionKind.RUN_LATER)
                .deliveryMode(DelayedActionDeliveryMode.INTERNAL_TURN).attempts(3).maxAttempts(4).build();
        when(delayedActionService.leaseDueActions(10)).thenReturn(List.of(action));
        when(delayedActionDispatcher.dispatch(action)).thenReturn(
                CompletableFuture.completedFuture(DelayedActionDispatcher.DispatchResult.retryable("temporary")));
    ...
    void shouldSkipTickWhenDelayedActionsAreDisabled() {
        ...
        DelayedSessionActionScheduler scheduler = new DelayedSessionActionScheduler(delayedActionService,
                mock(DelayedActionDispatcher.class), runtimeConfigService, Clock.fixed(NOW, ZoneOffset.UTC));
    ...
    void shouldMarkCompletedOnSuccessfulDispatch() {
        ...
        DelayedSessionAction action = DelayedSessionAction.builder().id("delay-complete")
                .kind(DelayedActionKind.RUN_LATER).deliveryMode(DelayedActionDeliveryMode.INTERNAL_TURN).attempts(0)
                .maxAttempts(4).build();
        when(delayedActionService.leaseDueActions(10)).thenReturn(List.of(action));
    ...
    void shouldMarkDeadLetterOnTerminalDispatchResult() {
        ...
        DelayedSessionAction action = DelayedSessionAction.builder().id("delay-terminal")
                .kind(DelayedActionKind.RUN_LATER).deliveryMode(DelayedActionDeliveryMode.INTERNAL_TURN).attempts(0)
                .maxAttempts(4).build();
        when(delayedActionService.leaseDueActions(10)).thenReturn(List.of(action));
        when(delayedActionDispatcher.dispatch(action)).thenReturn(
                CompletableFuture.completedFuture(DelayedActionDispatcher.DispatchResult.terminal("policy denied")));
    ...
    void shouldRetryWhenDispatchFutureFails() {
        ...
        DelayedSessionAction action = DelayedSessionAction.builder().id("delay-failed-future")
                .kind(DelayedActionKind.RUN_LATER).deliveryMode(DelayedActionDeliveryMode.INTERNAL_TURN).attempts(1)
                .maxAttempts(5).build();
        when(delayedActionService.leaseDueActions(10)).thenReturn(List.of(action));
        when(delayedActionDispatcher.dispatch(action))
        ...
        try {
            ...
            verify(delayedActionService, timeout(2000)).rescheduleRetry(eq("delay-failed-future"),
                    eq(NOW.plus(java.time.Duration.ofMinutes(2))), eq("Dispatch failure: queue down"));
        } finally {
            scheduler.shutdown();
        }
    }
    ...
    void shouldRetryWhenDispatchReturnsNullResult() {
        ...
        DelayedSessionAction action = DelayedSessionAction.builder().id("delay-null-result")
                .kind(DelayedActionKind.RUN_LATER).deliveryMode(DelayedActionDeliveryMode.INTERNAL_TURN).attempts(2)
                .maxAttempts(6).build();
        when(delayedActionService.leaseDueActions(10)).thenReturn(List.of(action));
        ...
        try {
            ...
            verify(delayedActionService, timeout(2000)).rescheduleRetry(eq("delay-null-result"),
                    eq(NOW.plus(java.time.Duration.ofMinutes(10))), eq("Dispatch returned no result"));
        } finally {
            scheduler.shutdown();
        }
    }
    ...
    void shouldUseOneHourBackoffForLaterRetries() {
        ...
        DelayedSessionAction action = DelayedSessionAction.builder().id("delay-hour-backoff")
                .kind(DelayedActionKind.RUN_LATER).deliveryMode(DelayedActionDeliveryMode.INTERNAL_TURN).attempts(3)
                .maxAttempts(10).build();
        when(delayedActionService.leaseDueActions(10)).thenReturn(List.of(action));
        when(delayedActionDispatcher.dispatch(action)).thenReturn(
                CompletableFuture.completedFuture(DelayedActionDispatcher.DispatchResult.retryable("retry again")));
        ...
        try {
            ...
            verify(delayedActionService, timeout(2000)).rescheduleRetry(eq("delay-hour-backoff"),
                    eq(NOW.plus(java.time.Duration.ofHours(1))), eq("retry again"));
        } finally {
            scheduler.shutdown();
        }
    }
