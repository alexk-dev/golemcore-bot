...
public class DelayedSessionActionService implements SessionDelayedActionProtectionPort {
    ...
    public DelayedSessionAction schedule(DelayedSessionAction candidate) {
        ...
        synchronized (lock) {
            ...
            int pendingForSession = countPendingForSessionLocked(normalized.getChannelType(),
    ...
    public DelayedSessionAction scheduleJobReadyNotification(DelayedJobReadyEvent event) {
        ...
        return schedule(DelayedSessionAction.builder().channelType(event.channelType())
                .conversationKey(event.conversationKey()).transportChatId(event.transportChatId()).jobId(event.jobId())
                .kind(DelayedActionKind.NOTIFY_JOB_READY).deliveryMode(deliveryMode).runAt(clock.instant())
                .maxAttempts(runtimeConfigService.getDelayedActionsDefaultMaxAttempts())
                .dedupeKey(buildJobReadyDedupeKey(event)).cancelOnUserActivity(false).createdBy("event:job_ready")
    ...
    public boolean hasPendingActions(String channelType, String conversationKey) {
        ...
        synchronized (lock) {
            pruneRetainedTerminalLocked(clock.instant());
            return actions.values().stream().filter(action -> !action.isTerminal())
    ...
    public int clearActiveActions(String channelType, String conversationKey) {
        ...
        String normalizedConversation = normalizeConversationKey(conversationKey);
        synchronized (lock) {
            List<String> actionIds = actions.values().stream().filter(action -> !action.isTerminal())
                    .filter(action -> matchesSession(action, normalizedChannel, normalizedConversation))
                    .map(DelayedSessionAction::getId).toList();
    ...
    public List<DelayedSessionAction> leaseDueActions(int limit) {
        ...
        synchronized (lock) {
            ...
            List<DelayedSessionAction> leased = actions.values().stream().filter(action -> !action.isTerminal())
    ...
    private int countPendingForSessionLocked(String channelType, String conversationKey) {
        return (int) actions.values().stream().filter(action -> !action.isTerminal())
                .filter(action -> matchesSession(action, channelType, conversationKey)).count();
    }
    ...
    private int removeActiveDuplicatesLocked(DelayedSessionAction normalized) {
        ...
        List<String> duplicateIds = actions.values().stream().filter(action -> !action.isTerminal())
    ...
    private String buildDedupeIdentity(DelayedSessionAction action) {
        ...
        return String.join(":", "auto", channelType, conversationKey,
                action.getKind() != null ? action.getKind().name() : "unknown",
                action.getDeliveryMode() != null ? action.getDeliveryMode().name() : "unknown",
                normalizedValue(action.getJobId()), payloadValue(action, "message"),
                payloadValue(action, "instruction"), payloadValue(action, "artifactPath"),
                payloadValue(action, "artifactName"), payloadValue(action, "originalSummary"));
    }
    ...
    private DelayedSessionAction normalizeForCreate(DelayedSessionAction candidate, Instant now) {
        ...
        normalized.setId(
                StringValueSupport.isBlank(normalized.getId()) ? "delay-" + UUID.randomUUID().toString().substring(0, 8)
    ...
    private void pruneRetainedTerminalLocked(Instant now) {
        boolean changed = actions.values().removeIf(
                action -> action.isTerminal() && action.getExpiresAt() != null && !action.getExpiresAt().isAfter(now));
        if (changed) {
            persistLocked();
        }
    }
    ...
    private DelayedSessionAction copyAction(DelayedSessionAction source) {
        ...
        return DelayedSessionAction.builder().id(source.getId()).channelType(source.getChannelType())
                .conversationKey(source.getConversationKey()).transportChatId(source.getTransportChatId())
                .jobId(source.getJobId()).kind(source.getKind()).deliveryMode(source.getDeliveryMode())
                .status(source.getStatus()).runAt(source.getRunAt()).leaseUntil(source.getLeaseUntil())
                .attempts(source.getAttempts()).maxAttempts(source.getMaxAttempts()).dedupeKey(source.getDedupeKey())
                .cancelOnUserActivity(source.isCancelOnUserActivity()).createdBy(source.getCreatedBy())
                .createdAt(source.getCreatedAt()).updatedAt(source.getUpdatedAt()).completedAt(source.getCompletedAt())
                .expiresAt(source.getExpiresAt()).lastError(source.getLastError())
