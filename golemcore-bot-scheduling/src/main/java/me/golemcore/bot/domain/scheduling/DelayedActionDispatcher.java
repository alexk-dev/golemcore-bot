...
public class DelayedActionDispatcher {
    ...
    private final SessionRunDispatchPort sessionRunDispatchPort;
    ...
    public DelayedActionDispatcher(SessionRunDispatchPort sessionRunDispatchPort, ChannelRuntimePort channelRuntimePort,
            ToolArtifactReadPort toolArtifactReadPort, DelayedActionPolicyService policyService, Clock clock) {
        this.sessionRunDispatchPort = sessionRunDispatchPort;
    ...
    private CompletableFuture<DispatchResult> dispatchDirectMessage(DelayedSessionAction action) {
        if (!policyService.supportsProactiveMessage(action.getChannelType(), action.getTransportChatId())) {
            return CompletableFuture.completedFuture(
                    proactiveUnavailableResult(action.getChannelType(), "Proactive message delivery unavailable"));
        }
        ...
        return channel.sendMessage(action.getTransportChatId(), message)
                .handle((ignored, failure) -> failure == null ? DispatchResult.completed()
    ...
    private CompletableFuture<DispatchResult> dispatchDirectFile(DelayedSessionAction action) {
        if (!policyService.supportsProactiveDocument(action.getChannelType(), action.getTransportChatId())) {
            return CompletableFuture.completedFuture(
                    proactiveUnavailableResult(action.getChannelType(), "Proactive file delivery unavailable"));
        }
        ...
        try {
            download = toolArtifactReadPort.getDownload(artifactPath);
        ...
        return delivery.handle((ignored, failure) -> failure == null ? DispatchResult.completed()
    ...
    private CompletableFuture<DispatchResult> dispatchInternalTurn(DelayedSessionAction action) {
        if (!policyService.supportsDelayedExecution(action.getChannelType(), action.getTransportChatId())) {
            return CompletableFuture.completedFuture(
                    proactiveUnavailableResult(action.getChannelType(), "Delayed execution delivery unavailable"));
        }
        ...
        return sessionRunDispatchPort.submit(synthetic, null)
                .handle((ignored, failure) -> failure == null ? DispatchResult.completed()
    ...
    private Message buildInternalMessage(DelayedSessionAction action) {
        ...
        return Message.builder().id("delayed-" + action.getId()).role("user")
                .content(buildInternalMessageContent(action)).channelType(action.getChannelType())
                .chatId(action.getConversationKey()).senderId("internal:delayed-action").metadata(metadata)
