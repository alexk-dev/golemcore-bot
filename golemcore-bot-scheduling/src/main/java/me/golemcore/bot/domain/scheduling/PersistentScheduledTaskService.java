...
public class PersistentScheduledTaskService {
    ...
    public synchronized Optional<ScheduledTask> getScheduledTask(String scheduledTaskId) {
        ...
        return ensureLoaded().stream().filter(task -> scheduledTaskId.equals(task.getId())).findFirst();
    }
    ...
    public synchronized ScheduledTask createScheduledTask(String title, String description, String prompt,
        ...
        return createScheduledTask(title, description, prompt, reflectionModelTier, reflectionTierPriority,
                ScheduledTask.ExecutionMode.AGENT_PROMPT, null, null);
    }
    ...
    public synchronized ScheduledTask createScheduledTask(String title, String description, String prompt,
        ...
        ScheduledTask task = ScheduledTaskMutationSupport.createTask(title, description, prompt, reflectionModelTier,
                reflectionTierPriority, executionMode, shellCommand, shellWorkingDirectory);
    ...
    public synchronized ScheduledTask updateScheduledTask(String scheduledTaskId, String title, String description,
            String prompt, String reflectionModelTier, Boolean reflectionTierPriority) {
        return updateScheduledTask(scheduledTaskId, title, description, prompt, reflectionModelTier,
                reflectionTierPriority, null, null, null);
    }

    public synchronized ScheduledTask updateScheduledTask(String scheduledTaskId, String title, String description,
            String prompt, String reflectionModelTier, Boolean reflectionTierPriority,
            ScheduledTask.ExecutionMode executionMode, String shellCommand, String shellWorkingDirectory) {
        ScheduledTask task = requireScheduledTask(scheduledTaskId);
        ScheduledTaskMutationSupport.applyUpdate(task, title, description, prompt, reflectionModelTier,
                reflectionTierPriority, executionMode, shellCommand, shellWorkingDirectory);
    ...
    public synchronized Optional<ScheduledTask> findByLegacySource(String legacySourceType, String legacySourceId) {
        ...
        return ensureLoaded().stream().filter(task -> normalizedType.equals(task.getLegacySourceType()))
                .filter(task -> normalizedId.equals(task.getLegacySourceId())).findFirst();
    }
