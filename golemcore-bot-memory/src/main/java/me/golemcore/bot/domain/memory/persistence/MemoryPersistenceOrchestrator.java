    ...
    public void appendEpisodic(List<MemoryItem> items, String scope) {
        ...
        for (MemoryItem item : normalizedItems) {
            try {
                ...
                log.debug("[MemoryPersistence] Failed to serialize episodic item '{}': {}", item.getTitle(),
                        e.getMessage());
            }
        }
        if (payload.isEmpty()) {
        ...
        try {
            ...
            for (String line : lines) {
                ...
                try {
                    ...
                    item.setScope(MemoryScopeSupport
                            .normalizeScopeOrGlobal(item.getScope() != null ? item.getScope() : defaultScope));
                    items.add(item);
    ...
    private String toJsonl(List<MemoryItem> items) {
        ...
        for (MemoryItem item : items) {
            try {
                ...
                log.trace("[MemoryPersistence] Skipping non-serializable item '{}': {}", item.getId(), e.getMessage());
            }
        }
        return sb.toString();
    }
