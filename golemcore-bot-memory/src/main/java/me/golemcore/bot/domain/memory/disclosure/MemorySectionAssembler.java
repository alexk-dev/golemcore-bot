...
public class MemorySectionAssembler {
    ...
    private MemoryPackSection buildSection(MemoryDisclosurePlan plan,
            Map<MemoryItem.Layer, List<MemoryScoredItem>> grouped, MemoryPackSectionType sectionType) {
        return switch (sectionType) {
            case INDEX -> buildIndexSection(grouped);
            case WORKING_MEMORY ->
                    buildLayerSection(plan, grouped.get(MemoryItem.Layer.WORKING), MemoryItem.Layer.WORKING);
            ...
            case PROCEDURAL_MEMORY ->
                    buildLayerSection(plan, grouped.get(MemoryItem.Layer.PROCEDURAL), MemoryItem.Layer.PROCEDURAL);
    ...
    private Map<MemoryItem.Layer, List<MemoryScoredItem>> groupByLayer(MemoryDisclosurePlan plan) {
        ...
        for (MemoryScoredItem selected : plan.getSelectionResult().getSelectedCandidates()) {
            ...
            MemoryItem.Layer layer = selected.getItem().getLayer() != null ? selected.getItem().getLayer()
    ...
    private MemoryPackSection buildIndexSection(Map<MemoryItem.Layer, List<MemoryScoredItem>> grouped) {
        ...
        return MemoryPackSection.builder().type(MemoryPackSectionType.INDEX).title("Memory Index").lines(lines).build();
    }
    ...
    private MemoryPackSection buildLayerSection(MemoryDisclosurePlan plan, List<MemoryScoredItem> candidates,
        ...
        return MemoryPackSection.builder().type(sectionTypeFor(layer))
                .title(sectionTitleFor(layer, plan.getDisclosureMode())).lines(lines).build();
    }
    ...
    private MemoryPackSection buildDetailSection(MemoryDisclosurePlan plan) {
        ...
        return MemoryPackSection.builder().type(MemoryPackSectionType.DETAIL_SNIPPETS).title("Detail Snippets")
                .lines(lines).build();
    }
    ...
    private MemoryPackSection buildHintsSection(MemoryDisclosurePlan plan) {
        ...
        List<String> lines = List
                .of("Use memory_expand_section for deeper layer context and memory_read for full item details.");
        return MemoryPackSection.builder().type(MemoryPackSectionType.FOLLOWUP_HINTS).title("Follow-up Hints")
                .lines(lines).build();
    }
