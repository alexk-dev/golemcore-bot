 ...
 * <p>
 * The class intentionally reads as a short orchestration flow: normalize query, retrieve scored candidates, assemble a
...
public class MemoryContextOrchestrator {
    ...
    public MemoryAssemblyResult assemblePromptMemory(MemoryContextRequest request) {
        if (!runtimeConfigService.isMemoryEnabled()) {
            MemoryPack disabledPack = MemoryPack.builder().items(List.of()).sections(List.of())
                    .diagnostics(disabledPromptDiagnostics()).renderedContext("").build();
            return MemoryAssemblyResult.builder().query(request != null ? request.getQuery() : null)
                    .memoryPack(disabledPack).diagnostics(disabledPack.getDiagnostics()).build();
        }
        ...
        MemoryPack finalPack = MemoryPack.builder().items(structuredPack.getItems())
                .sections(structuredPack.getSections()).disclosureMode(structuredPack.getDisclosureMode())
                ...
                .build();
        return MemoryAssemblyResult.builder().query(normalized).scoredItems(scoredItems).memoryPack(finalPack)
                .diagnostics(finalPack.getDiagnostics()).build();
    }
    ...
    private MemoryPack safePromptPack(MemoryPack pack) {
        ...
        return MemoryPack.builder().items(List.of()).sections(List.of()).diagnostics(Map.of()).renderedContext("")
                .build();
    }
    ...
    private Map<String, Object> promptDiagnostics(MemoryQuery query, List<MemoryScoredItem> scoredItems,
