...
class MemoryLifecycleOrchestratorTest {
    ...
    void setUp() {
        ...
        memoryLifecycleOrchestrator = new MemoryLifecycleOrchestrator(runtimeConfigService,
                turnMemoryExtractionOrchestrator, memoryNormalizationService, memoryPersistenceOrchestrator,
                memoryPromotionOrchestrator);
    }

    @Test
    void shouldPersistEpisodicBeforePromotion() {
        TurnMemoryEvent event = TurnMemoryEvent.builder().scope("task:demo")
                .timestamp(Instant.parse("2026-03-24T12:00:00Z")).userText("user").assistantText("assistant").build();
        MemoryItem extracted = MemoryItem.builder().id("raw-1").content("raw").build();
        MemoryItem normalized = MemoryItem.builder().id("normalized-1").layer(MemoryItem.Layer.EPISODIC)
                .scope("task:demo").content("normalized").build();
    ...
    void shouldSkipLifecycleWorkWhenMemoryDisabled() {
        ...
        memoryLifecycleOrchestrator.persistTurnMemory(
                TurnMemoryEvent.builder().timestamp(Instant.now()).userText("user").assistantText("assistant").build());
    ...
    void shouldNormalizeAndUpsertProceduralItem() {
        ...
        MemoryItem source = MemoryItem.builder().id("proc-1").scope("task:demo")
                .content("Always double-check git status before committing").build();
        MemoryItem normalized = MemoryItem.builder().id("proc-1").layer(MemoryItem.Layer.PROCEDURAL).scope("task:demo")
                .content("Always double-check git status before committing").build();
    ...
    void shouldNormalizeAndUpsertSemanticItem() {
        MemoryItem source = MemoryItem.builder().id("sem-1").scope("task:demo").content("Remember this").build();
        MemoryItem normalized = MemoryItem.builder().id("sem-1").layer(MemoryItem.Layer.SEMANTIC).scope("task:demo")
                .content("Remember this").build();
