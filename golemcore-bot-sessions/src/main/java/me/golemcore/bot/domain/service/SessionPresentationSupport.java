...
public final class SessionPresentationSupport {
    ...
    public static SessionSummaryView toSummary(AgentSession session, boolean active) {
        ...
        return SessionSummaryView.builder().id(session.getId()).channelType(session.getChannelType())
                ...
                .title(buildTitle(session, conversationKey)).preview(buildPreview(session)).active(active).build();
    }
