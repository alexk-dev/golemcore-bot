...
@Command(name = "golemcore-bot", mixinStandardHelpOptions = true, versionProvider = RuntimeLauncherVersionProvider.class, sortOptions = false, description = {
        ...
        "A command is required. Use `web` to start the bundled web runtime or `cli` to run the CLI runtime."
}, footer = {
        "",
        "Examples:",
        "  golemcore-bot web --port=8080 --hostname=0.0.0.0",
        "  golemcore-bot web -J=-Xmx1g --port=9090",
        "  golemcore-bot cli --help",
...
final class LauncherCliArguments {
    ...
    private final CliCommand cliSubcommand = new CliCommand();
    ...
    LauncherArguments toLauncherArguments(String commandName) {
        if ("cli".equals(commandName)) {
            return toCliLauncherArguments();
        }
        return toWebLauncherArguments();
    }

    private LauncherArguments toWebLauncherArguments() {
        ...
        collectPassThroughArguments(webSubcommand.passThroughArguments, explicitJavaOptions, applicationArguments);
    ...
    private LauncherArguments toCliLauncherArguments() {
        List<String> explicitJavaOptions = new ArrayList<>(javaOptions);
        explicitJavaOptions.addAll(cliSubcommand.javaOptions);
        List<String> applicationArguments = new ArrayList<>();
        applicationArguments.add("cli");
        collectPassThroughArguments(cliSubcommand.passThroughArguments, explicitJavaOptions, applicationArguments);
        return new LauncherArguments(
                LauncherText.trimToNull(LauncherText.firstNonBlank(cliSubcommand.storagePath, storagePath)),
                LauncherText.trimToNull(LauncherText.firstNonBlank(cliSubcommand.updatesPath, updatesPath)),
                LauncherText.trimToNull(LauncherText.firstNonBlank(cliSubcommand.bundledJar, bundledJar)),
                null,
                null,
                List.copyOf(explicitJavaOptions),
                List.copyOf(applicationArguments));
    ...
    private void collectPassThroughArguments(
    ...
    @Command(name = "cli", sortOptions = false, description = {
            "Start the bundled CLI runtime.",
            "Launcher-specific options are handled locally; all unknown arguments",
            "are forwarded to the spawned CLI runtime."
    })
