...
public class MemoryDisclosurePlanner {
    ...
    public MemoryDisclosurePlan plan(MemoryDisclosureInput input) {
        return MemoryDisclosurePlan.builder()
                .selectionResult(input.getSelectionResult())
                .disclosureMode(input.getDisclosureMode())
                .promptStyle(input.getPromptStyle())
                .toolExpansionEnabled(input.isToolExpansionEnabled())
                .disclosureHintsEnabled(input.isDisclosureHintsEnabled())
                .detailMinScore(input.getDetailMinScore())
                .sectionTypes(memoryDisclosurePolicy.determineSections(input))
                .build();
    }
}
