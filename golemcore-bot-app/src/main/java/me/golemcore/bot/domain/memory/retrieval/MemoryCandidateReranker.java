 ...
 *
 * <p>
 * The first scoring pass optimizes for general prompt relevance. This reranker
 * refines the order using more precise signals such as exact title or content
 * phrase matches, title token coverage, and active skill alignment before
...
public class MemoryCandidateReranker {
    ...
    public List<MemoryScoredItem> rerank(MemoryRetrievalPlan plan, List<MemoryScoredItem> scored) {
        ...
        reranked.sort(Comparator
                .comparingDouble(RerankedCandidate::rerankScore)
                .reversed()
    ...
    private double contentPhraseBoost(MemoryQuery query, MemoryItem item, RerankingProfile profile) {
        return containsNormalizedPhrase(
                query != null ? query.getQueryText() : null,
                item != null ? item.getContent() : null)
                        ? profile.contentPhraseBoost()
                        : 0.0;
    }
    ...
    private record RerankedCandidate(
    ...
    private record RerankingProfile(
            double titlePhraseBoost,
            double contentPhraseBoost,
            double titleCoverageBoost,
