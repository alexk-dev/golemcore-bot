 ...
 *
 * <p>
 * The assembler centralizes verbosity handling so orchestrators can publish
 * stable diagnostics without embedding formatting policy into retrieval or pack
 * assembly services.
 */
@Service
public class MemoryDiagnosticsAssembler {
     ...
     */
    public Map<String, Object> buildPromptDiagnostics(
            MemoryQuery query,
    ...
    private void appendDetailedPromptDiagnostics(
        ...
        if (pack != null && pack.getSections() != null && !pack.getSections().isEmpty()) {
            diagnostics.put("sectionTypes", pack.getSections().stream()
