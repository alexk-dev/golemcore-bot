...
class MemoryDisclosurePolicyTest {
    ...
    void shouldIncludeDetailSectionForSelectiveDetailMode() {
        ...
        assertEquals(List.of(
                MemoryPackSectionType.WORKING_MEMORY,
                MemoryPackSectionType.EPISODIC_MEMORY,
                MemoryPackSectionType.SEMANTIC_MEMORY,
                MemoryPackSectionType.PROCEDURAL_MEMORY,
                MemoryPackSectionType.DETAIL_SNIPPETS,
                MemoryPackSectionType.FOLLOWUP_HINTS), sections);
    }
    ...
    void shouldNotExposeHintsInFullPackMode() {
        ...
        assertEquals(List.of(
                ...
                MemoryPackSectionType.EPISODIC_MEMORY,
                MemoryPackSectionType.SEMANTIC_MEMORY,
                MemoryPackSectionType.PROCEDURAL_MEMORY), sections);
    }

    private MemoryDisclosureInput input(MemoryDisclosureMode mode, boolean hintsEnabled) {
        return MemoryDisclosureInput.builder()
                .selectionResult(MemorySelectionResult.builder().build())
                .disclosureMode(mode)
                .promptStyle(MemoryPromptStyle.BALANCED)
                .toolExpansionEnabled(true)
                .disclosureHintsEnabled(hintsEnabled)
                .detailMinScore(0.8)
                .build();
    }
}
