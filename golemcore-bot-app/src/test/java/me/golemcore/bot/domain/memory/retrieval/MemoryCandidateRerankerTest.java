...
class MemoryCandidateRerankerTest {
    ...
    void shouldPreferExactTitleAndContentMatchesDuringSecondPass() {
        MemoryRetrievalPlan plan = MemoryRetrievalPlan.builder()
                ...
                .episodicLookbackDays(1)
                .build();

        MemoryScoredItem broadMatch = MemoryScoredItem.builder()
                .item(item("broad-match", "incident note", "redis connection issue"))
                .score(0.96)
                .build();
        MemoryScoredItem exactMatch = MemoryScoredItem.builder()
                .item(item(
                        "exact-match",
                        "redis connection reset playbook",
    ...
    void shouldKeepOriginalOrderWhenRerankingDisabled() {
        ...
        MemoryRetrievalPlan plan = MemoryRetrievalPlan.builder()
                .query(MemoryQuery.builder().queryText("redis connection reset").build())
                .requestedScope("global")
                .requestedScopes(List.of("global"))
                .episodicLookbackDays(1)
                .build();

        MemoryScoredItem first = MemoryScoredItem.builder()
                .item(item("first", "first note", "plain content"))
                .score(0.90)
                .build();
        MemoryScoredItem second = MemoryScoredItem.builder()
                .item(item("second", "redis connection reset playbook", "exact phrase"))
                .score(0.70)
                .build();
